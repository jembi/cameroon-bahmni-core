...
public class PatientGenderQueryHelper {
	...
	public String appendToWhereClause(String where){
		...
		String genderSearchCondition = getGenderSearchCondition(nameParameter);
		where = isEmpty(genderSearchCondition) ? where : combine(where, "and", enclose(genderSearchCondition));
		return where;
	}

	private String getGenderSearchCondition(WildCardParameter wildCardParameter) {
